...
_prep-env:
	if [ ! -f ./configs.env ]; then \
		...
		touch configs.env; \
		echo "NEXT_PUBLIC_USER_API_URL_LOCAL_SAM=http://localhost:3001/users" >> configs.env; \
		echo "NEXT_PUBLIC_DOMAIN=${NEXT_PUBLIC_DOMAIN}" >> configs.env; \
		echo "NEXT_PUBLIC_HOSTED_ZONE_NAME=${NEXT_PUBLIC_HOSTED_ZONE_NAME}" >> configs.env; \
		echo "NEXT_PUBLIC_HOSTED_ZONE_ID=${NEXT_PUBLIC_HOSTED_ZONE_ID}" >> configs.env; \
	fi
	if [ ! -f ./frontend/cdk-outputs.json ]; then \
		echo "No ./frontend/cdk-outputs.json file found, genereating from env variables"; \
		touch ./frontend/cdk-outputs.json; \
		echo "{" >> ./frontend/cdk-outputs.json; \
		echo "\"fake-deploy-outputs\": {" >> ./frontend/cdk-outputs.json; \
		echo "\"apiUrl\": \"https://execute-api.us-east-1.amazonaws.com/\"," >> ./frontend/cdk-outputs.json; \
		echo "\"siteDomain\": \"https://execute-api.us-east-1.amazonaws.com/\"," >> ./frontend/cdk-outputs.json; \
		echo "\"region\": \"us-east-1\"," >> ./frontend/cdk-outputs.json; \
		echo "\"userPoolClientId\": \"xxxxxadsfasdf\"," >> ./frontend/cdk-outputs.json; \
		echo "\"emailSubscribersDdb\": \"xxxxxadsfasdf\"," >> ./frontend/cdk-outputs.json; \
		echo "\"bucketName\": \"xxxxxadsfasdf\"," >> ./frontend/cdk-outputs.json; \
		echo "\"emailSubscribersAPGEndpointCC541036\": \"xxxxxadsfasdf\"," >> ./frontend/cdk-outputs.json; \
		echo "\"userPoolId\": \"xxxxxadsfasdf\"," >> ./frontend/cdk-outputs.json; \
		echo "\"identityPoolId\": \"xxxxxadsfasdf\"" >> ./frontend/cdk-outputs.json; \
		echo "}}" >> ./frontend/cdk-outputs.json; \
	fi
...
.PHONY: install
install: _check-aws-creds_configured _prep-env build-container install-infra install-frontend install-backend ## Initial setup - create config file, build container images, installs deps

.PHONY: test
test: test-frontend-lint test-frontend-ci test-backend-ci test-infra ## test the app - you can test specific parts with test-x (options are frontend, frontend-interactive,  infra)
...
_run-frontend:
	npm run dev --prefix frontend/ > /app/watch.log
...
_build:
	...
	./frontend/removeHtmlForS3Hosting.sh
	npm run sitemapGen --prefix frontend/
...
.PHONY: ci
genereate-sitemap: 
	${COMPOSE_RUN} make _genereate-sitemap

_genereate-sitemap:
	npm --prefix frontend/  run sitemapGen
...
.PHONY: install-backend-lambda-deps
install-backend-lambda-deps: 
	${COMPOSE_RUN} make _install-backend-lambda-deps

#ToDo: Find a better way to do this, rather have these in a layer
_install-backend-lambda-deps:
	cd backend/hello_world/ && pip install --target ./  -r requirements.txt
...
_install-backend:
	python3 -m venv backend/.venv && source backend/.venv/bin/activate && python3 -m pip install -r backend/tests/requirements.txt
...
_test-backend-unit-ci:
	source backend/.venv/bin/activate && cd backend && LOG_LEVEL=INFO AWSENV=AWSENV TABLE_NAME=emailSubscribers CORS_URL=http://localhost:3000 AWS_DEFAULT_REGION=us-east-1 python -m pytest tests/unit -v --cov=hello_world --cov-report xml --cov-fail-under 80 && cd ..
...
_test-backend-unit:
	source backend/.venv/bin/activate && cd backend && LOG_LEVEL=INFO AWSENV=AWSENV TABLE_NAME=emailSubscribers CORS_URL=http://localhost:3000 AWS_DEFAULT_REGION=us-east-1  ptw --runner "python -m pytest -v --cov=hello_world --cov-report xml --cov-fail-under 80 "
...
_check-infra-synthed:
	if [ ! -s ./${CDK_DIR}/template.yaml ]; then make _build && make _genereate-sitemap && make _synth; else echo "found template.yaml"; fi
...
synth-no-build: _prep-cache
	${COMPOSE_RUN} make _synth

synth: _prep-cache fix-docker-cdk-perms-issue
	${COMPOSE_RUN} make _synth
...
deploy-infra: _prep-cache fix-docker-cdk-perms-issue ## deploys project to AWS with the configs specified in configs.env
	${COMPOSE_RUN} make _deploy-infra

_deploy-infra: 
	cd ${CDK_DIR} && cdk deploy --require-approval never ${PROFILE} --outputs-file ./cdk-outputs.json 
	cd ..
	cp ./${CDK_DIR}/cdk-outputs.json ./frontend/

deploy-frontend: _prep-cache build ## deploys project to AWS with the configs specified in configs.env
	${COMPOSE_RUN} make _deploy-frontend

_deploy-frontend: 
	./frontend/deploySite.sh "${PROFILE}"
...
diff-no-build: _prep-cache ## an overview of what infra will be deployed (cdk diff)
	${COMPOSE_RUN} make _diff

diff: _prep-cache fix-docker-cdk-perms-issue ## an overview of what infra will be deployed (cdk diff)
	${COMPOSE_RUN} make _diff
